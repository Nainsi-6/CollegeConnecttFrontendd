...
const SavedPostsModal = ({ isOpen, onClose, userId }) => {
  ...
  const fetchSavedPosts = async () => {
    setLoading(true)
    try {
      const response = await fetch(`https://collegeconnectwebsite-1.onrender.com/api/posts/saved/${userId}`)
      if (response.ok) {
  ...
  const unsavePost = async (postId) => {
    try {
      const response = await fetch(`https://collegeconnectwebsite-1.onrender.com/api/posts/${postId}/unsave`, {
        method: "POST",
        headers: {
